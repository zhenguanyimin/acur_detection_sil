...
function [info,adc_data_deal_A,rdmap,rdmapAB,mtd_data_sumIQ_H,mtd_data_subIQ_H,mtd_data_ABIQ_H,DC1,comparePL] = getRdmapFromADC16_16bit_AB(namelist,iFlie,cfarFlag,rWin,dWin)
...
% if( abs(info.azimuth) < 10 || abs(info.azimuth) > 30 || abs(info.elevation) > elevationEnable )
% if( abs(info.azimuth) < 30 || abs(info.azimuth) > 50 || abs(info.elevation) > elevationEnable )
    adc_data_deal_A = -1;
    rdmap = -1;
    rdmapAB = -1;
    mtd_data_sumIQ_H = -1;
...
% diff_before = adc_data_deal_A(24,21) - adc_data_deal_B(24,21)
% diff_after = abs(adc_data_calibA(24,21) - adc_data_calibB(24,21))
...
% 
% figure,plot(abs(adc_data_calibA(24,:)));hold on;
% plot(abs(adc_data_calibB(24,:)));%%幅度
...
%% ===================== 1D FFT ======================%%
% for k=1:Ndoppler
%     fft_adc_data(k,:)=fft(adc_data_deal(k,:) .* win',Nrange)*2/Nrange;
% %     fft_adc_data(k,:)=fft(adc_data_deal(k,:) .* win',Nrange);
%     % % % fft_adc_data(k,:)=fftshift(fft(adc_data_deal(k,:)));
% end

%% 1D FFT对齐FPGA
rWin_int = round(rWin/max(rWin)*(2^10-1));%%10bit
rWin_int = [rWin_int(1,1);rWin_int(1,1);rWin_int(1:Nrange-2,:)];
...
for k=1:Ndoppler
   adc_data_tmp1 = adc_data_sumIQ(k,:) .* rWin_int';%%16bit+10bit = 26bit
...
%    adc_data_tmp3=floor(fft(adc_data_tmp2 ,Nrange));
...
for k=1:Ndoppler
   adc_data_tmp1 = adc_data_subIQ(k,:) .* rWin_int';%%16bit+10bit = 26bit
   adc_data_tmp2 = floor( adc_data_tmp1./ 2^11);%%floor( adc_data_tmp1./ 2^9),23bit,2^3,2^10,2^11,,2^10
   %% new add 1 
   ntBP = numerictype(1,16,0);
   x_BP = fi(adc_data_tmp2, 1, 23, 0) ;
...
for k=1:Ndoppler
   adc_data_tmp1 = adc_data_calibA(k,:) .* rWin_int';%%16bit+10bit = 26bit
   adc_data_tmp2 = floor( adc_data_tmp1./ 2^11);%%floor( adc_data_tmp1./ 2^9),23bit,2^3,2^10,2^11,,2^10
   %% new add 1 
   ntBP = numerictype(1,16,0);
   x_BP = fi(adc_data_tmp2, 1, 23, 0) ;
...
% %    fft_adc_data(k,:) = floor(adc_data_tmp3./2^8);%%2^8
   ...
   ntBP = numerictype(1,16,0);%%1st is sign￡?WordLength 16bit￡?FractionLength 0bit(EffectiveLength 15bit(last 8bit integer + front 7bit fraction))
   x_BP = fi(tmp1, 1, 16, 0) ;
   rfft_data_AIQ(k,:) = quantize(x_BP,ntBP);   
end

%% B路1DFFT
  rfft_data_BIQ = zeros(Ndoppler,Nrange);
for k=1:Ndoppler
   adc_data_tmp1 = adc_data_calibB(k,:) .* rWin_int';%%16bit+10bit = 26bit
   adc_data_tmp2 = floor( adc_data_tmp1./ 2^11);%%floor( adc_data_tmp1./ 2^9),23bit,2^3,2^10,2^11,,2^10
   %% new add 1 
   ntBP = numerictype(1,16,0);
   x_BP = fi(adc_data_tmp2, 1, 23, 0) ;
...
% %    fft_adc_data(k,:) = floor(adc_data_tmp3./2^8);%%2^8
   ...
   ntBP = numerictype(1,16,0);%%1st is sign￡?WordLength 16bit￡?FractionLength 0bit(EffectiveLength 15bit(last 8bit integer + front 7bit fraction))
   x_BP = fi(tmp1, 1, 16, 0) ;
   rfft_data_BIQ(k,:) = quantize(x_BP,ntBP);   
end
...
%     mtd_data(:,k)=fftshift(fft(data_tmp, Ndoppler));
% %     mtd_data(:,k)=fft(data_tmp, Ndoppler);
% end 


%% 2D FFT对齐FPGA
dwin_int = round(dWin*(2^15-1));
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for k=1:RANGE_BIN_ENABLE    
    data_temp1 = floor(dwin_int .* rfft_data_sumIQ(:,k));   %% 15bit + 16bit
...
%% 差路2DFFT
for k=1:RANGE_BIN_ENABLE    
    data_temp1 = floor(dwin_int.*rfft_data_subIQ(:,k));   %% 15bit + 16bit
    data_temp2 = floor(data_temp1/2^16);
...
%% A路2DFFT
for k=1:RANGE_BIN_ENABLE    
    data_temp1 = floor(dwin_int .* rfft_data_AIQ(:,k));   %% 15bit + 16bit
    data_temp2 = floor(data_temp1 / 2^16);
  
    input = data_temp2 ./ 2^15;
    [output, blkexp, overflow] = xfft_v9_1_bitacc_mex(generics, nfft, complex(input), scaling_sch, direction);
    mtd_data_AIQ(:,k) = output.'*2^15;
end 

%% B路2DFFT
for k=1:RANGE_BIN_ENABLE    
    data_temp1 = floor(dwin_int.*rfft_data_BIQ(:,k));   %% 15bit + 16bit
    data_temp2 = floor(data_temp1/2^16);
  
    input = data_temp2./2^15;
    [output, blkexp, overflow] = xfft_v9_1_bitacc_mex(generics, nfft, complex(input), scaling_sch, direction);
    mtd_data_BIQ(:,k) = output.'*2^15;
end 
...
%% 两通道相位差
% diff_after = (angle(mtd_data_AIQ(18,22)) - angle(mtd_data_BIQ(18,22))) * 180 / pi
