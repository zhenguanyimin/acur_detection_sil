...
Sys_Phasepole_MesureAngele = 0;%%计算测角相位极性,0
...
for i_tar = 1:NumTar
    ...
    %%A/B通道IQ提取
    A_i = real( mtd_data_AIQ(i_range,i_doppler) );%%实部
    A_q = imag( mtd_data_AIQ(i_range,i_doppler) );%%虚部
...
%     end
    ...
    %%更新方位角信息及属性信息
%     Target_Para_out(NumOutPara + 5,i_tar) = aziBeam + symbol_ZF * (AgeGap * Index);%%目标方位角=波位方位角+和差测得角偏，表的间隔是0.05度,度°
...
%         deltaTheta = 0;        
%     end
    Target_Para_out(NumOutPara + 7,i_tar) = aziBeam + (deltaTheta / pi * 180);%%目标方位角=波位方位角+和差测得角偏deltaTheta,度°
%     Target_Para_out(NumOutPara + 5,i_tar) = aziBeam + symbol_ZF * (deltaTheta / pi * 180);%%目标方位角=波位方位角+和差测得角偏deltaTheta,度°
    
       
    Target_Para_out(NumOutPara + 8,i_tar) = sqrt( sub_i2_sub_q2 );%%差路幅值
    Target_Para_out(NumOutPara + 9,i_tar) = sqrt( sum_i2_sum_q2 );%%和路幅值
